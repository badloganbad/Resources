...
{
	//set the bullet intital state
	active = false;

	//set bullet speed
	speed = 800.0;

	//loaad the texture into a surface
...
void Bullet::Update(float deltaTime) // float delta
{
	...
	if (active)
	{
		...
		//Update bullet position with the code to account for precision loss
		posRect.y = (int)(pos_Y + 0.5f);

		//check to see if the bullet is off the top of the screen
		// and deactive and move off screen
